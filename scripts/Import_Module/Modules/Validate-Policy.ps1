...
function Get-PolicyFromTable {
    ...
    try {
        ...
        if ($Action -eq "Create New Policy" -or $Action -eq "Update Policy" ) {
            $xlr_action = 'Export Policy'
            $query = "SELECT * FROM policies WHERE PolicyGuid = @PolicyId AND WorkflowID=@WorkFlowID AND ActionType=@xlr_action  AND Environment=@Source"
