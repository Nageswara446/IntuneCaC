function Fetch-PolicyFromGit {
    ...
    # Write-Host "DEBUG: Fetch-PolicyFromGit started with Source: $Source, Destination: $Destination, PolicyId: $PolicyId, gitpath: $gitpath"
    ...
    $releaseURL = $null
    # Write-Host "DEBUG: gitpath value: $gitpath"
    try {
        ...
        if (-not $Config.Git.GitPAT -or -not $Config.Git.repoOwner -or -not $Config.Git.repoName -or -not $Config.Git.rawUrl -or -not $Config.Git.ReleaseUrl) {
        ...
        if ($gitpath -match 'releases/download/(Release-[^/]+)') {
            ...
            # Write-Host "DEBUG: Extracted ReleaseVersion: $ReleaseVersion"
        } else {
        ...
        if ($gitpath -match '/([0-9a-fA-F\-]{36})\.json$') {
            ...
            # Write-Host "DEBUG: Extracted PolicyGuid: $PolicyGuid"
        } else {
        ...
        # Fetch release
        try {
            # Write-Host "DEBUG: Fetching release from URL: $releaseURL"
            $release = Invoke-RestMethod -Uri $releaseURL -Headers $headers -Method Get
            Write-Host "DEBUG: Release fetched successfully. Assets count: $($release.assets.Count)"
        } catch {
            # Write-Host "ERROR: Failed to fetch release info from GitHub. $($_.Exception.Message)"
            $result.ErrorMessage = "Failed to fetch release info from GitHub. $($_.Exception.Message)"
            return $result
        }

        if (-not $release.assets) {
            # Write-Host "ERROR: No assets found in release: $TagName"
        ...
        if (-not $asset) {
            # Write-Host "ERROR: Asset $AssetName not found for PolicyId $PolicyId"
            $result.ErrorMessage = "`n Asset ID not found for $($PolicyId)"
            return $result
        }

        $assetId = $asset.id
        # Write-Host "DEBUG: Found asset ID: $assetId for asset name: $AssetName"
        ...
        $DownloadPath = "C:\Users\HCL0733\OneDrive - Allianz\Desktop\WPS_INTUNE_CaC\scripts"
