...
param(
  ...
  [string]$Title    = "Policy Comparison Report ",
  [string]$Subtitle = "Snapshot",
...
# Import-Module .\Modules\Common
Import-Module "C:\URM\WPS_INTUNE_CaC\scripts\Modules\Common" -Force
...
function GenerateTableRows {
    ...
    foreach ($entry in $Data.Keys) {
        ...
        if ($value -is [System.Collections.IEnumerable] -and !$value.GetType().Name.Equals('String')) {
            $valueHtml = "<span class='toggle' onclick='toggleVisibility(this)'>Show/Hide</span><div class='hidden'>" + ($value -join ", ") + "</div>"
        } else {
            $valueHtml = $value
        }

        $rows += "<tr><td>$name</td><td>$valueHtml</td></tr>"
    }

    return $rows
}

if($srcObject.psobject.Properties.name -contains "name") {
$srcPolicyName=$srcObject.name
} else {
$srcPolicyName=$srcObject.displayName
}

if($dstObject.psobject.Properties.name -contains "name") {
$dstPolicyName=$dstObject.name
} else {
$dstPolicyName=$dstObject.displayName
}

$comparisonresults=Compare-IntunePolicyData -SrcObject $SrcObject -DstObject $DstObject
if($comparisonresults["status"] -eq "Match") {
$statushtml="no difference"
}elseif($comparisonresults["status"] -eq "Mismatch"){
$statushtml="difference"
}else{
$statushtml="unknown status"
}
$srchtml=GenerateTableRows -Data $comparisonresults['missing_in_src']
$dsthtml=GenerateTableRows -Data $comparisonresults['missing_in_dst']
...
<h3 align='center'> Result: There is $($statushtml) found between source and target policies</h3>
</div>
 
    <section class="grid">

     <div class='card2'>
<h3>Source Policy Details</h3>
<div class='kpi'>
<div class='value'>Tenant: $($srcTenant)</div>
<div class='value''>Policy Name: $($srcPolicyName)</div>
</div>
</div>

     <div class='card2'>
<h3>Destination Policy Details</h3>
<div class='kpi'>
<div class='value'>Tenant: $($dstTenant)</div>
<div class='value''>Policy Name: $($dstPolicyName)</div>
</div>
</div>
 
      <div class="card full">
<h3>Settings Missed at Source</h3>
<table>
<thead>
<tr>
...
<tbody>
 ...
 <div class="card full">
<h3>Settings Missed at Destination</h3>
<table>
<thead>
<tr>
<th>Name</th><th>Value</th>
